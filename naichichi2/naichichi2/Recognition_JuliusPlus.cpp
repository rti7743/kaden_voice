...
bool Recognition_JuliusPlus::checkDictation(Recog *recog,const OncSentence* sentence) const
{
	...
	{
		this->PoolMainWindow->SyncInvokeLog("#checkDictation: no-word",LOG_LEVEL_DEBUG);
		return false;
	}
	if (! checkYobikake( (*it3)->wordnode->yomiPress ) )
	{
		this->PoolMainWindow->SyncInvokeLog("#checkDictation: gomi word",LOG_LEVEL_DEBUG);
		return false;
	}
	...
	if(ret < 0)
	{
		this->PoolMainWindow->SyncInvokeLog("#checkDictation: j_open error",LOG_LEVEL_DEBUG);
		return false;
	}
...
void Recognition_JuliusPlus::OnOutputResult(Recog *recog)
{
	...
	if (allSentence.empty())
	{
		...
		this->PoolMainWindow->SyncInvokeLog("//スキップします" ,LOG_LEVEL_DEBUG);
		return ;
	}
