 ...
 *
 * <JA>
 * @brief  メモリ入力：WAV/RAWファイルをメモリから読み込みます
 *
 ...
 */
static boolean
myread(void *buf, size_t unitbyte, int unitnum)
{
  int tmp;

  tmp = unitbyte * unitnum;
  if (tmp > g_max - g_corrent) tmp = g_max - g_corrent;

  memcpy(buf,g_memory+g_corrent , tmp);
  g_corrent += tmp;
  if (tmp  < unitnum) {
    return(FALSE);
  }
...
#define MYREAD(A,B,C)  if (!myread(A, B, C)) {jlog("Error: adin_file: file is corrupted\n"); return -1;}
 ...
 * 
 * @param memory [in] memory pointer
 * @param memory_max [in] memory size
 * 
...
setup_wav()
{
  ...
  while (myread(dummy, 1, 4)) {
    ...
    for (i=0;i<len;i++) myread(dummy, 1, 1);
  }
 ...
 */
static boolean
adin_memory_open(void* memory,int memory_max)	/* NULL for standard input */
{
  FILE *fp;

  if (memory == NULL || memory_max <= 4) {
    jlog("Error: adin_memory: failed memory \n");
    return(FALSE);
  }
