...
#define SEXYTEST(uniq,name) SEXYTEST_IMP_(name,false,_SEXYTEST_LINE_STRCAT(SexyTest##uniq , __COUNTER__,__LINE__),false)
#define SEXYTEST_HEAVYTEST(uniq,name) SEXYTEST_IMP_(name,true,_SEXYTEST_LINE_STRCAT(SexyTest##uniq , __COUNTER__,__LINE__),false)
#define SEXYTEST_IMP_(name,heavy,guid,toppriority) \
	static SexyTestUnit<template struct guid> guid(false,#name,toppriority); \
	void SexyTestUnit<template struct guid>::test()
